#!/usr/bin/env python
"""Quick CLI tool to send a message to the agent and stream the response."""
...
def send_message_to_agent(message: str):
    ...
    if not agent:
        print(f"Error: {agent_display_name} agent not found")
        return
    
    print(f"Sending message to {agent_display_name}: {message}\n")
    print("=" * 50)
...
def main():
    parser = argparse.ArgumentParser(description="Send a quick message to the agent")
